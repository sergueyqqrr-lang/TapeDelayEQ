...
void TapeDelayEQAudioProcessor::processBlock (juce::AudioBuffer<float>& buffer, juce::MidiBuffer&)
{
    ...
    for (int ch = 0; ch < numChannels; ++ch)
    {
        ...
        for (int n = 0; n < numSamples; ++n)
        {
            ...
            //    Si el interruptor "EQ On" está apagado, se salta esta etapa
            //    pero el delay sigue sonando normal (sin EQ, no en silencio).
            const float delayedEQd = eqOn ? eq.processSample (delayed) : delayed;
            ...
            //    El motor de delay/feedback sigue corriendo SIEMPRE (aunque los
            //    interruptores de salida estén apagados), para que al reactivar
            //    "Original" o "Delay" no haya clics ni cortes raros.
            const float feedbackSignal = dry + delayedEQd * feedback;
            delayLine.pushSample (0, feedbackSignal);
            delayLine.advance();

            // 5) Mezcla final. Los interruptores dryOn/delayOn deciden qué
            //    parte suena en la salida; el mix sigue controlando la
            //    proporción entre las dos cuando ambas están encendidas.
            const float dryOut = dryOn   ? dry * (1.0f - mix) * dryGain : 0.0f;
            const float wetOut = delayOn ? wet * mix          * wetGain : 0.0f;
            channelData[n] = dryOut + wetOut;
        }
    }
}

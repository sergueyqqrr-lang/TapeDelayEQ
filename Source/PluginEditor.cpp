...
void TapeDelayEQAudioProcessorEditor::paint (juce::Graphics& g)
{
    ...
    g.setFont (16.0f);
    g.drawFittedText ("Tape Delay EQ", { 0, 0, getWidth(), 0 }, juce::Justification::centred, 1);
    // (el título de texto se quitó de la franja superior porque ahora ahí
    //  van los interruptores; si quieres un título visual, agrégalo en una
    //  franja aparte antes de toggleArea en resized())

    g.setFont (12.0f);
    g.setColour (juce::Colours::grey);
    g.drawFittedText ("EQ (solo señal de delay)", { 0, 235, getWidth(), 20 },
                       juce::Justification::centred, 1);
}

void TapeDelayEQAudioProcessorEditor::resized()
{
    // --- Franja superior: interruptores on/off ---
    auto toggleArea = getLocalBounds().removeFromTop (40).reduced (10, 8);
    const int toggleWidth = toggleArea.getWidth() / 3;
    dryOnButton.setBounds   (toggleArea.removeFromLeft (toggleWidth).reduced (4));
    delayOnButton.setBounds (toggleArea.removeFromLeft (toggleWidth).reduced (4));
    eqOnButton.setBounds    (toggleArea.removeFromLeft (toggleWidth).reduced (4));

    // --- Fila de controles del delay ---
    auto area = getLocalBounds().withTrimmedTop (80);
